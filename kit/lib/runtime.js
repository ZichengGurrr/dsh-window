...
export function apply(ctx, config) {
    ...
    if (process.platform !== 'win32') {
        logger.info('dsh-window-kit: non-Windows host, staying idle');
    }
        ...
        void ensureInstalled(resolved, nodeDeps())
            .then(result => {
            logger.info(`dsh-window-kit: app ready at ${result.exePath}`
                + `${result.downloaded ? ` (downloaded ${result.version})` : ''}`
                + `${result.updated ? ' (updated)' : ''}${result.shortcut ? ', shortcut refreshed' : ''}`);
        })
            .catch(error => { logger.warn(`dsh-window-kit: install failed: ${String(error)}`); });
    }
    ctx.tools.register(defineTool({
        name: 'desktop_launch',
        description: 'Launch the DeepSeek Harness desktop app (dsh-window, a native Windows WebView2 window around the webchat). '
